...
(SELECT  ec.subsidiary_code AS SUBSIDIARY, ec.subledger_code AS SUBLEDGER, CAST('' AS VARCHAR(16)) AS segment,YEAR(ec.closing_date)||'Q'||QUARTER(EC.closing_date)||'ILL'||COALESCE(seg.seg_code,'x') AS LOB, ec.currency_code, b.code, 'DSC' AS TYPE, ec.closing_date AS CLOSING_DATE
...
INNER JOIN (
	...
	AND c.sgmt_ll LIKE 'ILL%') seg ON seg.seg_ls = 'ILL'||COALESCE(ec.segment_code,'x')
WHERE 
pt.code = 'ZCR'
AND ec.lob_code IS NULL
AND ec.VALID_TO = '9999-12-31'
AND ec.SUPP_DATE = '9999-12-31'
GROUP BY ec.closing_date,ec.subsidiary_code, ec.subledger_code, YEAR(ec.closing_date)||'Q'||QUARTER(EC.closing_date)||'ILL'||COALESCE(seg.seg_code,'x'), ec.currency_code, b.code
);
...
DROP PROCEDURE SP_LOAD_WRK_APAC_PROJECTION_VALIDATION;

CREATE OR REPLACE PROCEDURE SP_LOAD_WRK_APAC_PROJECTION_VALIDATION (BIGINT,CHARACTER VARYING(64),CHARACTER VARYING(64),CHARACTER VARYING(64),CHARACTER VARYING(64)) RETURNS INTEGER 
LANGUAGE nzplsql AS 
BEGIN_PROC 
...
SELECT
	...
	CASE WHEN SRC.UNDERWRITING_ORDER = 1 AND SRC.ENDORSEMENT_NUMBER = 0 THEN CAST(SRC.AMOUNT * POS.SIGN_CONVENTION AS DECFLOAT)
		ELSE CAST(SRC.AMOUNT * POS.SIGN_CONVENTION * POS.RETRO_FACTOR AS DECFLOAT)	END AS AMOUNT,
	SRC.CREATED_BY,
	SRC.CREATED_DATE,
	SRC.LINE_NUMBER,
	SRC.REQUEST_ID,
	...
	WHERE
		...
		FROM
			...
			SELECT
				ERR.ERROR_ROW
			FROM
				DELIVERY_<env>.UPLOAD_ERROR_LOG ERR
			WHERE
				ERR.REQUEST_ID = ' || P_REQUEST_ID || '
				AND ERR.ERROR_MESSAGE_ID IN (
				SELECT
					ERROR_MESSAGE_ID
				FROM
					DELIVERY_<env>.ERROR_MESSAGE
				WHERE
					MESSAGE_TYPE_FLAG = ''F''
